  ...
  */
    ...
    async function loadPerks({ search = '', merchant = '' } = {}) {
      ...
      try {
        ...
        const res = await api.get('/perks/all', {
          params: {
            ...
            // Only include merchant param if provided
  ...
  // ==================== EVENT HANDLERS ====================
    ...
    function handleSearch(e) {
      ...
      loadPerks({ search: searchQuery, merchant: merchantFilter })
    }
    ...
    function handleReset() {
      ...
      setSearchQuery('')
      setMerchantFilter('')
      // Load unfiltered list immediately
      loadPerks({ search: '', merchant: '' })
    }

    // Initial load: fetch perks once on mount
    useEffect(() => {
      loadPerks({ search: '', merchant: '' })
    ...
    useEffect(() => {
      // small debounce to avoid flooding server when typing
      const t = setTimeout(() => {
        loadPerks({ search: searchQuery, merchant: merchantFilter })
      }, 450)

      return () => clearTimeout(t)
      // intentionally not including loadPerks in deps to avoid ref re-creation
      // eslint-disable-next-line react-hooks/exhaustive-deps
